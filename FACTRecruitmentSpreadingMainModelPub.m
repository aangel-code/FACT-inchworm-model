...
% Full model capable of capturing many features of the WT and 
% chd1\Delta strains
...
for rctr=1:1:Realisations
    ...
    %set up a timer for each simulation realisation
    realtics = tic;
    
    %Reinitialise state storage and counters:
    FACTStates = zeros(MaxFACTNo,4);
    FACTNo = 0;
    ExtNo = 0;
    RetNo = 0;
    ...
    for tctr=1:1:relaxationSteps
        ...
        if diceRoll < FBRN
            %Attempt a new FACT binding at the start
            if (FACTNo == 0) || (FACTStates(1,1) > NucFootprint)
                for ictr=MaxFACTNo:-1:2
                    FACTStates(ictr,:) = FACTStates(ictr-1,:);
                end
                FACTStates(1,:) = [1 NucFootprint NucFootprint 1];
                FACTNo = FACTNo + 1;
                ExtNo = ExtNo + 1;
    ...
    for tctr=1:1:timeSteps
        ...
        if diceRoll < FBRN
            %Attempt a new FACT binding at the start
            if (FACTNo == 0) || (FACTStates(1,1) > NucFootprint)
                for ictr=MaxFACTNo:-1:2 %Seems to work with older Matlab!
                    FACTStates(ictr,:) = FACTStates(ictr-1,:);
                end
                FACTStates(1,:) = [1 NucFootprint NucFootprint 1];
                FACTNo = FACTNo + 1;
